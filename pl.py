...
async def scrape_service_link(browser, service_link, state_link, city_link, all_data):
    ...
    try:
        ...
        df.to_csv('/content/drive/MyDrive/Random/scraped_next_data1252435521.csv', index=False)
        await page.close()
...
async def get_next_data():
    ...
    try:
        ...
        df = pd.DataFrame(all_data)
        df.to_csv('/content/drive/MyDrive/Random/scraped_next_data_error14545445.csv', index=False)
